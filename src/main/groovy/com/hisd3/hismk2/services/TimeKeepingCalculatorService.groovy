...
//        sample
        ...
        while (date.isBefore(endDate)) {
            ...
            normalAccumulatedLog.inTime = firstLog.inTime
            normalAccumulatedLog.outTime = firstLog.outTime
            normalAccumulatedLog.date = firstLog.date
            ...
            normalAccumulatedLog.schedule.title = firstLog.scheduleTitle
            ...
            schedule.each {
                ...
                overtimeLog.inTime = secondLog.inTime
                overtimeLog.outTime = secondLog.outTime
                overtimeLog.date = secondLog.date
                ...
                overtimeLog.schedule.title = secondLog.scheduleTitle
            ...
            String regularOICFormattedDate = "${formattedDate}_OIC"
            if (employeeSchedule.get(regularOICFormattedDate)) {
                ...
                oicLogFinal.inTime = oicLog.inTime
                oicLogFinal.outTime = oicLog.outTime
                oicLogFinal.date = oicLog.date
                ...
                oicLogFinal.schedule.title = oicLog.scheduleTitle
                oicLogFinal.schedule.type = "OIC"
                if (employeeSchedule.get(regularOICFormattedDate)) {
                    oicLogFinal.department = departmentRepository.findById(employeeSchedule.get(regularOICFormattedDate)['department'] as UUID).get()
                }
            ...
            //===Leaves Schedules===
            String leaveScheduleFormattedDate = "${formattedDate}_LEAVE"
            if (employeeSchedule.get(leaveScheduleFormattedDate)) {
                ...
                leavesLogFinal.schedule.title = leavesLog.scheduleTitle
                leavesLogFinal.schedule.type = "LEAVE"
                if (employeeSchedule.get(leaveScheduleFormattedDate)) {
                    leavesLogFinal.department = departmentRepository.findById(employeeSchedule.get(leaveScheduleFormattedDate)['department'] as UUID).get()
                }
...
//            }
            ...
            summary.totals.isAbsentOnly = totals.getIsAbsentOnly()
            summary.totals.isEmpty = totals.getIsEmpty()
            summary.totals.isOvertimeOnly = totals.getIsOvertimeOnly()
            summary.totals.isRestDayOnly = totals.getIsRestDayOnly()

            summary.totalsOriginal = objectMapper.convertValue(totals, Totals)
    ...
    EmployeeAccumulatedAttendanceDto getEmployeePerformanceSummary(UUID id, Instant startDate, Instant endDate) {
        List<AccumulatedLogSummary> summaryList = getAccumulatedLogs(id, startDate, endDate)

        EmployeeAccumulatedAttendanceDto employeePerfSummary = new EmployeeAccumulatedAttendanceDto()
        summaryList.each {
            employeePerfSummary = addAccumulatedLogs(employeePerfSummary, objectMapper.convertValue(it.totals, EmployeeAccumulatedAttendanceDto))
        }
    ...
    EmployeeAccumulatedAttendanceDto calculateAccumulatedLog(
        ...
        if (schedule && schedule.label != "R") {
            ...
            accumulatedLog.scheduleStart = schedule.dateTimeStartRaw
            accumulatedLog.scheduleEnd = schedule.dateTimeEndRaw
            accumulatedLog.scheduleTitle = schedule.title
        ...
        schedule.each { EmployeeScheduleDto currentSchedule ->
            ...
            for (it in logs) {
                ...
                if (computedHours.worked > 0) {
                    ...
                    accumulatedLog.scheduleStart = currentSchedule.dateTimeStartRaw
                    accumulatedLog.scheduleEnd = currentSchedule.dateTimeEndRaw
                    accumulatedLog.scheduleTitle = currentSchedule.title
                    accumulatedLog.date = date
