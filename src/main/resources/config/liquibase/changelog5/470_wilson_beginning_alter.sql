...
AS WITH beg AS (
         SELECT di.id,
           ...
           FROM inventory.department_item di
             LEFT JOIN inventory.inventory_ledger il ON il.item = di.item AND il.source_dep = di.department AND il.document_types = '0caab388-e53b-4e94-b2ea-f8cc47df6431'::uuid
