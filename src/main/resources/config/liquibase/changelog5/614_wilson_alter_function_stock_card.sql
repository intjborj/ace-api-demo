...
BEGIN
  ...
  select
	...
	e.desc_long,
	a.reference_no,
	(a.ledger_date + interval '8 hours') as ledger_date,
	case
		...
		case
			...
			case
				...
				else coalesce(round((sum(a.ledger_unit_cost * (sum(a.ledger_qty_in - a.ledger_qty_out))) over (order by a.ledger_date)) / nullif((sum(a.ledger_qty_in - a.ledger_qty_out) over (order by a.ledger_date)), 0), 2), a.ledger_unit_cost) end as wcost,
