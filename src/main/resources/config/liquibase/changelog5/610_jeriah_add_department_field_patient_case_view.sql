...
SELECT DISTINCT p.id,
    ...
    c.originating_hci,
    array_to_string(array_agg(DISTINCT e2.id), ','::text) AS managing_staffs,
    array_to_string(array_agg(DISTINCT e3.id), ','::text) AS comanaging_physicians,
    c.diet,
    c.entry_datetime,
...
LEFT JOIN ( SELECT DISTINCT m_2.id,
            m_2.employee,
            m_2."case",
            m_2.created_by,
            m_2.created_date,
...
LEFT JOIN hrm.employees e2 ON e2.id = m2.employee
LEFT JOIN ( SELECT DISTINCT m_3.id,
            m_3.employee,
            m_3."case",
            m_3.created_by,
            m_3.created_date,
...
WHERE m_3.is_main = false OR m_3.is_main is null or m_3.is_main = true) m3 ON m3."case" = c.id
LEFT JOIN hrm.employees e3 ON e3.id = m3.employee
GROUP BY p.id, c.id, c.status, c.discharge_condition, c.discharge_disposition, e.first_name, e.last_name, e.id, m."position", c.admission_datetime, c.transferred_in, c.transferred_out, c.registry_type, c.may_go_home_datetime, c.room, c.service_type, c.transfer_hci, c.discharged_datetime, c.originating_hci, c.diet, c.admitting_diagnosis, c.entry_datetime, p.dob, p.gender, c.department
