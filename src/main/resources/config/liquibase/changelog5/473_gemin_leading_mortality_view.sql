CREATE OR REPLACE VIEW pms.mortality_by_age_view2 AS (
WITH patient_age AS (
         SELECT p_1.id,
            p_1.gender,
            date_part('year'::text, age(c.entry_datetime, p_1.dob::timestamp with time zone))::integer AS page,
            c.discharge_disposition
           FROM pms.patients p_1
           left join pms.cases c on c.patient = p_1.id
           where c.id is not null and c.discharge_disposition is not NULL and c.discharge_condition = 'EXPIRED'
        )
