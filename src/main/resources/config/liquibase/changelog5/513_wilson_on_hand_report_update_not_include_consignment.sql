 DROP FUNCTION IF EXISTS inventory.onhand_report;
 CREATE OR REPLACE FUNCTION inventory.onhand_report(filter_search character varying, dep_id uuid, filterdate date)
 RETURNS TABLE(id uuid, item uuid, desc_long character varying, unit_of_purchase character varying, unit_of_usage character varying, category_description character varying, department uuid, department_name character varying, onhand bigint, last_unit_cost numeric, last_wcost numeric, expiration_date date)
 LANGUAGE plpgsql
AS $function$
BEGIN
  RETURN QUERY
  SELECT
	...
	FROM
		...
		a.department = dep_id and a.is_assign = true and b.active = true and (b.consignment = false or b.consignment is null) order by b.desc_long;
END;
$function$
;