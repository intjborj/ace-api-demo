...
AS SELECT bid.billingitem,
   ...
   FROM billing.billing_item_details bid
            ...
            left join public.departments  dept on dept.id = svc.department
...
CREATE OR REPLACE VIEW inventory.inventoryref
AS SELECT department_item.id,
          department_item.item,
          department_item.department
   FROM inventory.department_item;

CREATE OR REPLACE VIEW billing.billingitem_inventory_view_with_department as
SELECT bid.billingitem,
       bid.field_value,
       inv.id as inventoryid,
       dept.id as inventory_department,
       dept.parent_department  as inventory_parentdepartment
FROM billing.billing_item_details bid
         LEFT JOIN inventory.inventoryref inv ON inv.id = bid.field_value::uuid
         left join public.departments  dept on dept.id = inv.department
WHERE bid.field_name::text = 'INVENTORYID'::text;


CREATE OR REPLACE VIEW billing.billingitem_simpleinventory_view_with_department as
SELECT bid.billingitem,
       bid.field_value,
       inv.id as inventoryid,
       inv.department as inventory_department
FROM billing.billing_item_details bid
         LEFT JOIN inventory.inventoryref inv ON inv.id = bid.field_value::uuid
WHERE bid.field_name::text = 'INVENTORYID'::text;
