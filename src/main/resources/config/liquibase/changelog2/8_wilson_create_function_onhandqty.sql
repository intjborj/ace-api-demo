drop FUNCTION if exists "inventory"."onhand";
CREATE FUNCTION	inventory.onhand(IN depId uuid, IN itemId uuid)
RETURNS integer
LANGUAGE 'plpgsql'
AS $$
DECLARE qty integer;
BEGIN
		...
		WITH stockcard as(
		    ...
		    a.ledger_qty_out,
		    a.ledger_unit_cost
		)SELECT runningQty as onHand FROM stockcard ORDER BY ledger_date DESC LIMIT 1);
		RETURN qty;
END; $$