-- View: pms.surgical_procedure_by_age_view

-- DROP VIEW pms.surgical_procedure_by_age_view;

CREATE OR REPLACE VIEW pms.surgical_procedure_by_age_view
 AS
 WITH patient_age AS (
         SELECT p_1.id,
            p_1.gender,
 ...
 SELECT d.value ->> 'rvsCode'::text AS icdcode,
  ...
  GROUP BY (d.value ->> 'rvsCode'::text), (d.value ->> 'longName'::text), (date_part('year'::text, c.entry_datetime))
  ORDER BY (count(*)) DESC;
