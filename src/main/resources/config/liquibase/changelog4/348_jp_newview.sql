CREATE OR REPLACE FUNCTION pms.is_json(
...
CREATE OR REPLACE VIEW pms.morbidity_view
 AS
select
	d->>'diagnosisCode' as icd_code,
	d->>'longName' as long_name,
	date_part('year'::text, c.entry_datetime) AS reporting_year,
	count(d->>'diagnosisCode') as patient_count
from pms.cases c,
json_array_elements(case when (pms.is_json(icd_diagnosis) and icd_diagnosis is not null) then icd_diagnosis::json else null end) d
group by icd_code, long_name, reporting_year order by patient_count desc;

-------------------------------------------------------------------------------------

-- View: pms.morbidity_by_age_view

-- DROP VIEW pms.morbidity_by_age_view;

CREATE OR REPLACE VIEW pms.morbidity_by_age_view
 AS
 WITH patient_age AS (
         SELECT p_1.id,
            p_1.gender,
